...
class PIDController(Node):
    def __init__(self):
        ...
        self.goal_tolerance = 0.25  # khoảng cách threshold để coi là đạt goal
    ...
    def genbot_callback(self, msg: String):
        ...
        if msg.data == "dance1":
            ...
            self.robot_mode = 3
            self.current_task = "hungry" 
            self.current_goal = [7.8, -0.925, 0.0]
            self.send_goal(self.current_goal[0], self.current_goal[1], self.current_goal[2]) 
        elif msg.data == "sofa": 
            self.robot_mode = 3
            self.current_task = "sofa" 
            self.current_goal = [2.95, 1.5, 0.85]
            self.send_goal(self.current_goal[0], self.current_goal[1], self.current_goal[2])       
    ...
    def timer_callback(self):
        ...
        if self.robot_mode == 2:
            ...
            if self.current_goal is not None:
                ...
                odom_tf = self.get_transform('map', 'base_link')  # pose hiện tại
                ...
                if distance < self.goal_tolerance:
                    ...
                    if self.current_task == "sofa":
                        self._publish_status("reached_sofa")
                        self.current_task = None 
                        self.current_goal = None
                        # self.robot_mode = 4 
                        self.robot_mode = 1
                    else:
                        # 其他任務 (如 hungry, tool) 照常回程
